...
fixed, and only $\alpha$ is averaged.  The answer to this fixed-origin,
fixed-time problem is as follows.
...
$O(\varepsilon L^{1-D/2})=o(1)$ if $D>2$.  The map
$z\mapsto z\chi(z/(\varepsilon L))$ has uniformly bounded Lipschitz norm on
...
uniform error $L^{-A-4}$ and polynomial degree and coefficient norm, applies
...
$z_{i+2}=a_{m-i}z_{i+1}+z_i$.  Moreover,
...
$k$.  Both endpoint pairs are therefore bounded in terms of the fixed
vectors $k$ and $\ell$, so
Lemma~\ref{lem:endpoint-recurrence} applies.  Thus, for fixed nonzero
$k,\ell$, the resonance relation is
...
\begin{proof}
First take a finite digit-cylinder function times a torus character, and let
$d$ be the largest initial digit depth on which the chosen test depends.  For
the raw digit block in \eqref{eq:actual-window}, $d=j+R+1$; after replacing
complete quotients by a fixed number $M$ of future digits below, one may have
$d\le j+R+M$.  Partition into the countably many complete depth-$d$
cylinders, grouped by the finitely many local block words occurring in the
test.  By \eqref{eq:block-character-reduction}, on each such cylinder the coefficients
...
$t>d$ for all sufficiently large $n$, uniformly in $j$.  Retain the coarser complete depth-$j$
cylinder union on which $q_j\ge\e^{\lambda j-\delta H}$ and, below every
retained depth-$d$ prefix, the complete depth-$t$ descendants on
...
\[
 q_t^2\le\e^{-cH}n|Q_j|.
\]
Lemma~\ref{lem:descendant}, with prefix depth $d$, therefore makes every
nonzero character
...
window laws.  These truncated tests are covered by the preceding argument
with $d\le j+R+M$.  For fixed $M,K$ only finitely many words remain; first let
...
real-valued cylinder--torus polynomials $P_{R,M}$ that are finite real linear
combinations of the monomials in \eqref{eq:block-monomial}, of finite radii
